...
- **Bulk VM Actions**: Select multiple VMs on the VMs page via row checkboxes (or select-all in header). A bulk action bar appears showing the count of selected VMs with contextual Start/Stop/Reboot buttons that show how many applicable VMs each action targets. Actions execute sequentially with a progress indicator. Confirmation dialog before execution. Viewers cannot see checkboxes. Individual row actions are disabled during bulk operations.
- **Mounted Media / Eject ISO**: On the VM detail page, QEMU VMs with mounted CD/DVD media (ISOs) show a "Mounted Media" panel displaying the drive name and ISO filename with an "Eject" button. Ejecting sets the drive to `none,media=cdrom` via the Proxmox config API. Panel auto-hides when no media is mounted. LXC containers skip this panel entirely. API routes: `GET /api/vms/:id/media`, `POST /api/vms/:id/media/:drive/unmount`. Drive name validated against `ide|sata|scsi` prefix pattern.
...
- **User Activity Tracking**: Records last login time and full session history per user. DB: `lastLoginAt` column on `users` table updated on every login; `user_sessions` table stores userId, loginAt, logoutAt, ipAddress, userAgent. On login, a session record is created and `lastLoginAt` is updated. On logout, the session's `logoutAt` is set. Users list shows "Last login X ago" per user card. User detail page shows Last Login info card and a "Session History" panel with up to 50 most recent sessions, each displaying login time, active/ended status, IP address, browser type, and session duration. API: `GET /users/:id/sessions` returns session history.
