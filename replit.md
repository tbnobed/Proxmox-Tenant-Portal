...
- **PWA / Chrome Install**: The portal is installable as a Chrome app. `site.webmanifest` includes `id`, `start_url`, `scope`, description, and 192/512 icons; a minimal service worker (`public/sw.js`, registered in `main.tsx`) uses a network-first strategy and never caches `/api`, WebSocket, or auth traffic.
